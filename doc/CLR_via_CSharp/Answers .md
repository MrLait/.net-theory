...
таблицей.
		...
		SQL, Транзакции(ACID). Проблемы параллельного доступа с использованием транзакций и уровни изоляции транзакций в SQL
		Server (READ UNCOMMITTED | READ COMMITTED | REPEATABLE READ | SNAPSHOT | SERIALIZABLE). Optimistic and pessimistic lock.
...
принципам ACID, то есть быть атомарными(Atomicity), согласованность(Consistency), изолированными(Isolation) и
...
стороны других транзакций на то время, пока вы с ней работаете.
		...
		Базовый синтаксис SQL запроса, Select, AS, функции, DISTINCT, WHERE, IS NULL, BETWEEN, IN, Оператор LIKE, ESCAPE-символ,
		оператор ORDER BY, GROUP BY, Агрегатные функции, HAVING, Порядок выполнения SQL запроса
...
используя: LEFT JOIN => UNION ALL => RIGHT JOIN => WHERE левая_таблица.ключ IS NULL
	...
	Получение данных, не относящихся к левой и правой таблицам одновременно (обратное INNER JOIN):
		...
		табличное выражение, оператор WITH, Объединение запросов, оператор Union, INTERSECT, EXCEPT
...
идентификатор в поле company.
	DROP TABLE - предназначен для удаление таблицы: DROP TABLE [IF EXIST] имя_таблицы;
		...
		Масштабирование SQL баз данных. Репликация, подход master-slave, Шардинг, Вертикальный и Горизонтальный шардинг, Индексы
		в SQL. Понимать в чем разница кластерного от некластерного индекса. Некластерные индексы: составной индекс, уникальный
		индекс и покрывающий индекс, Понимать каким образом индекс облегчает поиск. Какие минусы индексов
...
которые не умещаются на одном сервере.
		...
		Основы индексов в SQL Server, Кучи, Кластеризованный и Некластеризованный индекс. Составной индекс, Уникальный индекс,
		Покрывающий индекс.
